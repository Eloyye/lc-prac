...
import { getProblem, getProblemForAttemptImport } from "./problems";
...
function createAttemptRecord(
  ...
  includeInactiveProblem: boolean,
): AttemptCreationResult {
  const existing = db.select().from(attempts).where(eq(attempts.id, values.id)).get();
  if (existing !== undefined) {
    if (existing.userId !== userId) return { kind: "conflict" };
    const best = bestForRow(db, userId, existing);
  ...
  const problem = includeInactiveProblem
    ? getProblemForAttemptImport(db, values.problemId, userId)
    : getProblem(db, values.problemId, userId);
  ...
  const best = bestForRow(db, userId, row);
...
export function createAttempt(
  ...
  return db.transaction((tx) => createAttemptRecord(tx as Db, userId, values, false));
}

/** Add one validated local Attempt inside the caller's account-Import transaction. */
export function importAttempt(
  db: Db,
  userId: string,
  values: CreateAttemptValues,
): AttemptCreationResult {
  return createAttemptRecord(db, userId, values, true);
}
